...
num_nets =  ih$nets_num_household
num_persons = ih$household_num
...
slept.seq =  rep( c(rep(0,8), rep(1,8)), 2)
num_nets.seq = rep(0:7, 4)
num_persons.seq = rep( seq(1, 15, 2) , 4)
...
ggplot( pred, aes(slept.seq, pred.p)) + geom_line() + 
    geom_ribbon(aes( ymin = X5., ymax = X94.), alpha = .2) +
    scale_x_continuous( breaks = 0:1) +
    facet_wrap( num_nets.seq ~ num_persons.seq)
```
