...
fit1a.lmer = glmer( hml3235 ~ slept + nets_house +  slept*nets_house + (1 | country_year_id ), 
                    family = binomial(link = "logit"),
                   data = d)
...
slept.seq =  c(rep(0,8), rep(1,8))
nets_house.seq = rep(0:7, 2)

link.fit1a.lmer = function( slept, nets_house){ 
    post$Intercept + post$slept*slept + post$nets_house*nets_house + 
        post$`slept:nets_house`*slept*nets_house
}

mu = mapply( link.fit1a.lmer, slept.seq, nets_house.seq)

pred.p = exp(apply( mu, 2, mean))
pred.PI = exp( apply( mu, 2, PI ))

pred = data.frame( slept.seq, nets_house.seq, pred.p, t( pred.PI ) )

ggplot( pred, aes(slept.seq, pred.p) ) + geom_line() + 
    scale_x_continuous( breaks = 0:1) +
    facet_wrap(~ nets_house.seq)
```
...
d = data_frame( hml3235, slept, nets_person, country_year_id) 
...
fit1b.lmer = glmer( hml3235 ~ slept + nets_person + + slept*nets_person + (1 | country_year_id ), 
                    family = binomial(link = "logit"),
                   data = d)
...
summary(ih$nets_num_household)
summary( ih$household_num )
summary(nets_person)
...
slept.seq =  c(rep(0,8), rep(1,8))
nets_person.seq = rep( seq(0,1.4, .2), 2)

post = extract.samples( fit1b.lmer )
link.fit1b.lmer = function( slept, nets_person){ 
    post$Intercept + post$slept*slept + post$nets_person*nets_person + 
        post$`slept:nets_person`*slept*nets_person
}
mu = mapply( link.fit1b.lmer, slept.seq, nets_person.seq)

pred.p = exp(apply( mu, 2, mean))
pred.PI = exp(apply( mu, 2, PI ))

pred = data.frame( slept.seq, nets_person.seq, pred.p, t( pred.PI ) )

ggplot( pred, aes(slept.seq, pred.p)) + geom_line() + 
        scale_x_continuous( breaks = 0:1) +
    facet_wrap(~ nets_person.seq)
```
