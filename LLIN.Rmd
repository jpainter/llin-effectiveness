---
title: "LLIN Effectiveness"
subtitle: "Personal and community effectiveness amond children under 5 years of age"
author: "Mateusz Plucinski and John Painter"
date: "`r Sys.Date()`"
output:
  tint::tintHtml: 
     self_contained: TRUE
...
To assess the individual- and community-level effects of LLIN use among children in household surveys during the last 10 years.  LLIN are the leading intervention for malaria control in Subsaharan Africa and is important to understand the magnitude of protection confered by LLIN and whether the protective effect has waned over time, perhaps due to insecticide resistance. 

# Objective

To analyse DHS survey data inorder to estimate the protective effect of LLIN as a direct effect of sleeping under the net and as an indirect effect of having nets in the community.  To control for other community effects, we construct a nested multi-level model of children within households; households within survey sample clusters; clusters within regions.

# Framework

```{r hypothetical1, fig.margin=TRUE, fig.width=4, fig.height=4, cache=TRUE, echo=FALSE}

ideal_net_law = data_frame( `Slept under net` = rep( c("Y","N"), 1),
                            label = c("Slept under net", "Did not sleep under net"),
                            prevalence = c(.3, .6),
                            community_effect = c(-.0, -.0)
                            ) 

library(ggrepel)
ggplot(ideal_net_law) + geom_abline( aes(intercept = prevalence, 
                                        slope = community_effect,
                                        color = `Slept under net`
                                        ),
                                     size = 2
                                     ) +
    geom_text( aes(label = label), x= .5, y=c(.3, .6), size = 6) +
    scale_x_continuous("Percent of population with a net", limits = 0:1, breaks = seq(0,1,.2)) +
    scale_y_continuous("Malaria Prevalence", limits = c(0,.8), breaks = seq(0,1,.2)) +
    guides( color = FALSE ) +
    ggtitle("No Coomunity effect", 
            subtitle = "As number of nets in community increases,\nthe relative risk between net users and non-users is the same") 
    
```


```{r hypothetical2, fig.cap= "Hypothetical protective effect of LLIN", fig.margin=TRUE, fig.width=4, fig.height=4, cache=TRUE, echo=FALSE}

ideal_net_law = data_frame( `Slept under net` = rep( c("Y","N"), 1),
                            label = c("Slept under net", "Did not sleep under net"),
                            prevalence = c(.3, .6),
                            community_effect = c(-.1, -.3)
                            ) 
...
ggplot(ideal_net_law) + geom_abline( aes(intercept = prevalence, 
    ...
    ggtitle("Community effect", 
            subtitle = "As number of nets in community increases,\nthe incidence drops declines among those using and not-using nets, indicating a community effect") 
